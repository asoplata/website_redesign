...
- [ ] TODO make my own reference to the proximal / distal
...
2. Inside of the inside of the tab, click the button `Load external drives (0)`, then select the file [OnlyRhythmicProx.json](https://raw.githubusercontent.com/jonescompneurolab/hnn-data/refs/heads/main/network-configurations/OnlyRhythmicProx.json) which you downloaded previously in [Section 2](#toc-2).
...
<td>
TODO: change image links
![](/Users/austinsoplata/rep/brn/jones-website-upstream/images/textbook/content/06_alpha_beta/images/initial-external-drives-look-01.png)
...
<td>
TODO: change image links
![](/Users/austinsoplata/rep/brn/jones-website-upstream/images/textbook/content/06_alpha_beta/images/initial-external-drives-look-02.png)
...
<td>
TODO: change image links
![](/Users/austinsoplata/rep/brn/jones-website-upstream/images/textbook/content/06_alpha_beta/images/initial-external-drives-look-03.png)
...
![](/Users/austinsoplata/rep/brn/jones-website-upstream/images/textbook/content/06_alpha_beta/images/image08.png)
</td>
<td>
...
TODO: change image links
![](/Users/austinsoplata/rep/brn/jones-website-upstream/images/textbook/content/06_alpha_beta/images/OnlyRhythmicProx-initial.png)
</div>

Concerning external drive stochasticity: As shown in the red histogram in the top panel of [Figure 7](#figure-7) above, with this parameter set, a burst of proximal input spikes is provided to the network at a rate of ~10 Hz (i.e., every 100 ms). Due to the stochastic nature of the inputs (controlled by the `Start time dev (ms)` and `Burst std dev (Hz)` parameters), there is some variability in the histogram of proximal input times, and the exact histogram pattern may look different on your simulation. Note that a decrease in the `Burst std dev (Hz)` would create shorter duration bursts (i.e., more synchronous bursts); this will be explored further in [Section 6.2](#toc-6-2) below.

The main point with this simulation is that ~10 Hz bursts of proximal drive induces current flow **up** the pyramidal neuron dendrites. This upward current flow increases the dipole signal above the 0 nAm baseline (making it positive), before the dipole relaxes back to zero approximately every 100 ms. This is observed in the blue current dipole waveform in the GUI window. See [Figure 3](#figure-3) and [Figure A on the Workshop page][] for illustration of the relationship between proximal drive input and the flow of current.

<a id="toc-4-3"></a>

### 4.2 Visualize the spectrogram of the net current dipole

To view the time-frequency spectrogram for this waveform (example in [Figure 8](#figure-8)), do the following steps:
...
TODO: change image links
![](/Users/austinsoplata/rep/brn/jones-website-upstream/images/textbook/content/06_alpha_beta/images/OnlyRhythmicProx_Spect.png)
</div>

The bottom panel of [Figure 8](#figure-8) shows the corresponding time-frequency spectrogram for this waveform, which exhibits a high-power continuous 10 Hz signal. Importantly, in this example, the strength of the proximal input spikes were titrated to be subthreshold; i.e., our simulated cortical cells do **not** spike in this instance. This is because we assume that macroscale oscillations are generated primarily by subthreshold current flow across large populations of synchronous pyramidal neurons (see [Section 1 Background](#toc-1) above for details). In [Section 6.2](#toc-6-2) below, we explore differences in the signal when the cells are driven to spike (see also [our ERP tutorial](../05_erps/erps_in_gui.html)).
...
### 6.3 Simulating and averaging multiple trials with jittered start times creates the impression of continuous oscillations
...
To first test the effects of jittering due to "Burst stdev" and averaging across trials, we will use a param file ([AlphaAndBetaJitter50.json](https://raw.githubusercontent.com/jonescompneurolab/hnn-data/refs/heads/main/network-configurations/AlphaAndBetaJitter50.json)) with rhythmic proximal and distal inputs provided at 10 Hz, with proximal and distal inputs in phase. These are the same parameters as in the [AlphaAndBeta.json](https://raw.githubusercontent.com/jonescompneurolab/hnn-data/refs/heads/main/network-configurations/AlphaAndBeta.json) file ([Section 6.2](#toc-6-2) above), but now with 10 trials instead of 1.
...
Follow the previous steps from [Section 6.2](#toc-6-2) to create a time-frequency spectrogram. The output will look like the following:
...
To run this simulation, click Start Simulation from the main GUI window. Follow the steps from [Section 6.2](#toc-6-2) again to create a time-frequency spectrogram.
...
### 6.4 Viewing network spiking activity
...
Return to the single-trial parameter set as in [Section 6.1](#toc-6-1) and [Section 6.2](#toc-6-2), by loading the
...
We described above ([Section 6](#toc-6)) that the timing of proximal and distal inputs can lead to either alpha events (when the bursts arrive to the local network out of phase) or beta events (when the bursts arrive in phase).
...
Next, we will test how these parameter changes affect the simulation. Select the `Simulation` tab and click the `Run` button. Then follow the steps from [Section 6.1](#toc-6-1) or [Section 6.2](#toc-6-2) to create a time-frequency spectrogram Once completed, you will see output in the GUI similar to that shown below.
...
First, notice that the histogram profile of the distal input bursts (green) are narrower corresponding to more synchronous input than in the prior simulation ([Section 6](#toc-6)). Second, notice that the waveform of the oscillation is different with a sharper downward deflecting signal, due to to the stronger distal input. These deflections increased ~20 Hz beta activity, as seen in the corresponding spectrogram (compare to spectrogram in [Section 6.1](#toc-6-1)). The 20 Hz frequency is set by the duration of the downward current flow, which with this parameter set is approximately 50 ms (see [@sherman_neural_2016] for further details).
...
<!-- ### 9.2 Increasing the strength (post-synaptic conductance) of the distal drive further creates high frequency responses due to induced spiking activity -->
...
<!-- ### 9.3 Increasing the delay between the proximal and distal inputs to anti-phase (50 ms delay) creates continuous alpha oscillations without beta activity -->
